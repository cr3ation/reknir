...
invoice_status_enum = postgresql.ENUM(
    "draft", "sent", "paid", "partial", "overdue", "cancelled", name="invoicestatus", create_type=False
)
...
def upgrade() -> None:
    ...
    op.create_index(
        op.f("ix_supplier_invoices_our_invoice_number"), "supplier_invoices", ["our_invoice_number"], unique=False
    )
