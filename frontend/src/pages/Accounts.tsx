...
export default function Accounts() {
  ...
  // Calculate opening balance validation
  // Assets (1xxx) should equal Liabilities + Equity (2xxx)
  const openingBalanceAssets = accounts
    .filter(a => a.account_number >= 1000 && a.account_number < 2000)
    .reduce((sum, a) => sum + a.opening_balance, 0)

  const openingBalanceLiabilitiesEquity = accounts
    .filter(a => a.account_number >= 2000 && a.account_number < 3000)
    .reduce((sum, a) => sum + a.opening_balance, 0)

  // In Swedish accounting, liability/equity accounts have negative opening balance (credit)
  // So: Assets + (-Liabilities) should = 0
  // If not zero, the difference needs to be added to equity
  ...
  const handleAutoFixOpeningBalance = async () => {
    if (!balanceResultAccount || !selectedCompany) return

    try {
      // The difference needs to be added as negative (credit) to equity
      const newBalance = balanceResultAccount.opening_balance - openingBalanceDifference
      await api.patch(`/accounts/${balanceResultAccount.id}`, {
        opening_balance: newBalance,
      })
      showMessage('Ingående balans korrigerad på konto 2091', 'success')
      await loadAccounts()
    } catch (error) {
      console.error('Failed to auto-fix balance:', error)
      showMessage('Kunde inte korrigera ingående balans', 'error')
    }
  }
  ...
  return (
    <div>
      ...
      {!isOpeningBalanceValid && openingBalanceAssets !== 0 && (
        <div className="mb-6 p-4 bg-yellow-50 border border-yellow-300 rounded-lg">
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              <h3 className="text-lg font-semibold text-yellow-800">Ingående balans balanserar inte</h3>
              <div className="mt-2 text-sm text-yellow-700 space-y-1">
                <p>
                  <strong>Tillgångar (1xxx):</strong>{' '}
                  {openingBalanceAssets.toLocaleString('sv-SE', { style: 'currency', currency: 'SEK' })}
                </p>
                <p>
                  <strong>Skulder + Eget kapital (2xxx):</strong>{' '}
                  {(-openingBalanceLiabilitiesEquity).toLocaleString('sv-SE', { style: 'currency', currency: 'SEK' })}
                </p>
                <p className="font-semibold text-yellow-900 pt-2">
                  Differens:{' '}
                  {openingBalanceDifference.toLocaleString('sv-SE', { style: 'currency', currency: 'SEK' })}
                </p>
              </div>
